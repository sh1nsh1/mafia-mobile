...
class GameRepository:
    ...
    async def create_game(self, admin_id: str, max_players: int = 4) -> GameModel:
        ...
        async with self.redis.pipeline(transaction=True) as pipe:
            ...
            await pipe.hset(
                self.LOBBY_KEY.format(lobby_id=lobby.id), mapping=lobby.to_dict()
            )

            # Добавляем админа в участники лобби
            await pipe.sadd(
                self.LOBBY_PARTICIPANTS_KEY.format(lobby_id=lobby.id), lobby.admin_id
            )
            ...
            await pipe.expire(
                self.LOBBY_PARTICIPANTS_KEY.format(lobby_id=lobby.id), self.LOBBY_TTL
            )
    ...
    async def update_lobby_max_players(self, lobby_id: str, max_players: int) -> bool:
        ...
        updated = await self.redis.hset(
            self.LOBBY_KEY.format(lobby_id=lobby_id), "max_players", str(max_players)
        )
